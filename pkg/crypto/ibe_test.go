...
func testFullIBEZeroPointAttack(t *testing.T) {
	...
	appPrivKey_affine, err := bls.NewG1PointFromCompressedBytes(appPrivKey.CompressedBytes)
	require.NoError(t, err)

	zeroG2 := types.G2PointZero()
	zeroG2_affine, err := bls.NewG2PointFromCompressedBytes(zeroG2.CompressedBytes)
	require.NoError(t, err)
...
func testFullIBEPairingProperties(t *testing.T) {
	...
	require.NoError(t, err)
	masterPK_affine, err := bls.NewG2PointFromCompressedBytes(masterPubKey.CompressedBytes)
	require.NoError(t, err)
	...
	// Convert to affine
	aQ_ID_affine, _ := bls.NewG1PointFromCompressedBytes(aQ_ID.CompressedBytes)
	bMasterPK_affine, _ := bls.NewG2PointFromCompressedBytes(bMasterPK.CompressedBytes)
	...
	zeroG1 := types.G1PointZero()
	zeroG1_affine, err := bls.NewG1PointFromCompressedBytes(zeroG1.CompressedBytes)
	require.NoError(t, err)
...
func testFullIBEInvalidMasterKey(t *testing.T) {
	...
	zeroMasterPK := types.G2PointZero()
