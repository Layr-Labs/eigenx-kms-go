...
type Protocol interface {
	GenerateShares() (map[common.Address]*fr.Element, []types.G2Point, error)
	VerifyShare(share *fr.Element, commitments []types.G2Point) bool
	FinalizeKeyShare(shares map[common.Address]*fr.Element, allCommitments [][]types.G2Point, participantIDs []common.Address) *types.KeyShareVersion
}
...
func NewDKG(nodeAddress common.Address, threshold int, operators []*peering.OperatorSetPeer) *DKG {
	return &DKG{
...
func (d *DKG) GenerateShares() (map[common.Address]*fr.Element, []types.G2Point, error) {
...
func (d *DKG) VerifyShare(share *fr.Element, commitments []types.G2Point) bool {
	...
	// Verify: share * G2 == Σ(commitment_k * nodeAddress^k)
	leftSide, err := crypto.ScalarMulG2(crypto.G2Generator, share)
	if err != nil {
		return false
	}

	jFr := bls.AddressToFr(d.nodeAddress)
...
func (d *DKG) FinalizeKeyShare(shares map[common.Address]*fr.Element, allCommitments [][]types.G2Point, participantIDs []common.Address) *types.KeyShareVersion {
	...
	if len(allCommitments) > 0 {
		expectedLen := d.threshold
		combinedCommitments = make([]types.G2Point, expectedLen)
