...
func TestVerifyAttestation(t *testing.T) {
	...
	for _, provider := range providers {
		t.Run("valid attestation with jwt verification", func(t *testing.T) {
			...
			require.Equal(t, []string{"/usr/local/bin/compute-source-env.sh", "npm", "start"}, claims.Args)
			require.Equal(t, "Never", claims.RestartPolicy)
			require.Equal(t, "18.20.8", claims.Env["NODE_VERSION"])
		})
